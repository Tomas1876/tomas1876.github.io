...
<BR>
<BR>
스코프 체인 상에 있는 변수라 해서 무조건 접근 가능한 것은 아니다. 위 코드에서 식별자 a는 전역과 inner 함수 내부에 각각 선언되어 있다. inner 함수 내부에서 a에 접근하려고 하면 무조건 스코프 체인 상의 첫번째 인자 즉 inner 스코프의 LexicalEnvironment부터 검색할 수 없다. 이때 a를 찾는다면 스코프 체인 검색을 중단하고 즉시 inner LexicalEnvironment가 가진 aㄹ르 반환하기 때무에 전역 공간에 선언한 동일한 이름의 a에는 접근할 수 없어진다. 이를 _변수 은닉화_ 라고 한다.
